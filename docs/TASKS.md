...
- [x] T-06: 不正レベル値テスト — `init_game(0)`, `init_game(7)` でエラーを返すことを検証
- [x] T-07: 全レベルAI着手テスト — Level 1〜6 で正常に着手が返ることを検証
- [x] T-08: AI決定性テスト — 同一局面・同一レベルで同じ手が返ることを検証
- [x] T-10: `init_game` 再呼び出しテスト — 2回目の `init_game()` で状態がリセットされることを検証
- [x] T-11: AIパス局面テスト — AI合法手0の局面で自動パスしプレイヤー手番に戻ることを検証
- [x] T-12: 完全読みタイムアウトテスト — 5秒超過時にフォールバック手を返すことを検証
- [x] T-13: AI決定性（100回）テスト — 同一局面・同一レベルで100回連続同じ手を返すことを検証
- [x] 未終局 `get_result()` エラーテスト — ゲーム未終了時に `get_result()` がエラーを返すことを検証
...
- [x] CSS: ブレークポイント 768px、盤面最小幅 352px（44px×8マス）
- [x] モバイル: 縦積みレイアウト、タップ領域 44px×44px 保証

### 4-7. 画面統合

- [x] `App.tsx` — 画面遷移（タイトル ↔ ゲーム ↔ 結果）
- [x] `init_game` エラー時のUI対応（モデル読み込み失敗メッセージ表示・開始ボタン無効化・再試行導線）
...
- [x] rust-test: `cargo test`（T-01〜T-05, T-09）
