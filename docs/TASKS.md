...
- [ ] `Board` 構造体（`black: u64`, `white: u64`）
- [ ] 初期盤面生成 `new()`
- [ ] 合法手ビットマスク計算 `legal_moves(is_black)`
- [ ] 8方向反転＋着手 `place(pos, is_black)` → 反転ビットマスク返却
- [ ] 石数カウント `count()`, 空きマス数 `empty_count()`
- [ ] 1次元配列変換 `to_array()` — `[u8; 64]`
- [ ] 単体テスト: T-01（初期合法手4マス）、反転処理の正当性、石数カウント
