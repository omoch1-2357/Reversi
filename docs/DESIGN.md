...
pub struct NTupleEvaluator {
    /// タプル定義: 各タプルのマス位置リスト
    tuples: Vec<Vec<u8>>,
    /// phase 数（version 1 読み込み時は 1、version 2/3 は 30）
    phase_count: usize,
    /// 重み配列: weights[phase][tuple][index]
...
impl NTupleEvaluator {
    ...
    /// phase = min((60 - empty_count) / 2, phase_count - 1) を選び、
    /// その phase の重みだけで D4 対称変換（回転4 + 反射4）を合算する
...
```text
盤面に対して以下の8つの対称変換を適用し、各変換後のタプルインデックスで重みを引く:
1. 恒等変換
2. 90度回転
3. 180度回転
4. 270度回転
...
Python実装では `np.rot90(...)` と `np.fliplr(...)` を組み合わせて同等の D4 変換を生成する。
```
...
pub fn from_bytes(data: &[u8]) -> Result<Self, String> {
    ...
    //    - num_tuples を読み取り
    //    - data_crc32 を読み取り
    //    - version 2/3 の場合は reserved を phase_count として読む
    ...
    // 4. 重み配列の読み取り
    //    - version 1: 単一phaseとして各タプルの 3^tuple_size 個の f32 を読む
    //    - version 2/3: phase-major で各 phase / 各タプルの f32 を読む
...
│       └── ai/
│           ├── mod.rs           # AI公開インターフェース
│           ├── ntuple.rs        # NTupleEvaluator, D4対称変換, weights.binデシリアライズ
