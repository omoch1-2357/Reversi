...
pub struct NTupleEvaluator {
    /// タプル定義: 各タプルのマス位置リスト
    tuples: Vec<Vec<u8>>,
    /// phase 数（version 1 読み込み時は 1、version 2 は 30）
    phase_count: usize,
...
impl NTupleEvaluator {
    ...
    /// その phase の重みだけで回転4対称変換（0/90/180/270度）を合算する
...
pub fn from_bytes(data: &[u8]) -> Result<Self, String> {
    ...
    //    - num_tuples を読み取り
    //    - data_crc32 を読み取り
    //    - version 2 の場合は reserved を phase_count として読む
    ...
    //    - version 2: phase-major で各 phase / 各タプルの f32 を読む
