...
※ version 2 の phase_count は 30 とし、phase = min((60 - empty_count) / 2, 29)
```
...
| version: 2 | 30 phase N-Tuple。`reserved` に `phase_count` を格納 |
| 互換方針 | Rust推論ローダは version 1 / 2 の両方を受け入れる |
...
- 今回採用するタプル集合（14パターン）では、回転4対称（0/90/180/270度）で評価し、
