...
| F-04 | 石の反転表示 | 石が裏返る際に反転結果を表示する（アニメーションは必須としない） |
...
| NF-04 | UI応答性 | AI思考中もUIが固まらない（Web Worker使用） |
...
pub struct GameState {
    ...
    pub flipped: Vec<u8>,      // 反転した石の位置リスト（UI反映用）
}
...
│  - reserved: u32                    │
│    - version 1: 未使用(0)           │
│    - version 2: phase_count         │
│    - version 3: phase_count         │
├─────────────────────────────────────┤
...
│ Weights (可変長)                    │
│  version 1:                         │
│  - 各タプル: [f32; 3^tuple_size]    │
│  version 2 / 3:                     │
│  - 各 phase, 各タプルの順で          │
│    [f32; 3^tuple_size] を連結        │
│    (phase-major, リトルエンディアン) │
└─────────────────────────────────────┘

※ data_crc32: Tuple Definitions + Weights 部分のCRC32チェックサム
※ version 2 / 3 の phase_count は 30 とし、phase = min((60 - empty_count) / 2, 29)
```
...
```mermaid
graph LR
    PY[Python CLI<br/>train.py] -->|PyO3| RTRAIN[Rust 学習コア]
    RTRAIN -->|バイナリ出力 + CRC32付与| BIN[weights.bin]
    BIN -->|"include_bytes!()"| RUST[Rust ビルド]
    RUST -->|wasm-pack| WASM["reversi_bg.wasm<br/>(モデル内蔵)"]
```

1. **学習時（Python CLI + Rust core）**: Rust 学習コアが `weights.bin` を生成し、Python 側はCLIと検証を担当する
...
| バージョン | 対応 |
|-----------|------|
| version: 1 | 単一phaseの初期リリースフォーマット |
| version: 2 | 回転4対称の 30 phase N-Tuple。`reserved` に `phase_count` を格納 |
| version: 3 | D4対称（回転4 + 反射4）の 30 phase N-Tuple。`reserved` に `phase_count` を格納 |
| 互換方針 | Rust推論ローダは version 1 / 2 / 3 のすべてを受け入れる |
...
- 今回採用するタプル集合（14パターン）では、D4対称（回転4 + 反射4）で評価する
- 複数のタプルパターンを定義し、それらの重みの合計で盤面を評価する
- 重みは 30 phase に分割し、2k 手目と 2k+1 手目を同一phaseとして扱う
- phase は phase = min((60 - empty_count) / 2, 29) で決定し、パスでは進まない
- 学習はRust実装のTD-Lambda自己対戦によりゲーム結果から更新する
